    ...
    <List v-loading="loading" @change="onChange">
          ...
          <div class="header_row">
            <el-form :inline="true" :model="searchForm" style="display: inline-flex; align-items: center;"
              ...
              <el-form-item label="图纸名称:">
                <el-input v-model="searchForm.fileName" placeholder="请输入" style="flex: 1; min-width: 80px;"
                  clearable></el-input>
              </el-form-item>
              <el-form-item label="图纸ID:">
                <el-input v-model="searchForm.userFileId" placeholder="请输入" style="flex: 1; min-width: 80px;"
                  clearable></el-input>
              </el-form-item>
              <el-form-item>
      ...
      <el-table :data="tableData" v-if="isList">
        ...
        <el-table-column prop="status" label="状态">
          <template slot-scope="scope">
            <span class="status"
              :class="scope.row.fileStatus === '1' ? 'status' : scope.row.fileStatus === '2' ? 'status3' : 'status1'">{{
                status[scope.row.fileStatus] }}</span>
          </template>
        </el-table-column>
        <el-table-column fixed="right" label="操作" width="122">
          <template slot-scope="scope">
            <div class="operateBtn">
              <el-button type="text" class="blueText" @click="toProject(scope.row)"
                :disabled="scope.row.fileStatus !== '1'">打开图纸</el-button>
              <el-dropdown>
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  <el-dropdown-item v-if="scope.row.status == 2 && scope.row.isSingle === 'true'"
                    @click.native="onCopy(scope.row.fileUrl)">复制URL</el-dropdown-item>
                  <el-dropdown-item v-if="scope.row.fileStatus == '1'"
                    @click.native="share(scope.row)">分享</el-dropdown-item>
      ...
      <div class="listBox" v-if="!isList">
        <div class="listItem" v-for="(item, index) in tableData" :key="index">
          ...
          <div class="flexBetween">
            ...
            <div class="status"
              :class="item.fileStatus === '1' ? 'status' : item.fileStatus === '2' ? 'status3' : 'status1'">
              {{ status[item.fileStatus] }}</div>
          </div>
          <div class="flexBetween">
            <div>
              <el-button class="blueBtn" type="primary" size="small" :disabled="item.fileStatus !== '1'"
                @click="toProject(item)">进入项目</el-button>
            </div>
            <el-dropdown>
              <span class="el-dropdown-link">
                <i class="el-icon-arrow-down el-icon-more"></i>
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item v-if="item.status == 2 && item.isSingle === 'true'"
                  @click.native="onCopy(item.fileUrl)">复制URL</el-dropdown-item>
                <el-dropdown-item v-if="item.fileStatus == '1'" @click.native="share(item)">分享</el-dropdown-item>
      ...
      <el-empty v-if="!isList && !tableData.length" :image="require('@/assets/noData.png')"
        :image-size="100"></el-empty>
      <Pagination :total="total" :page.sync="pages.pageNo" :limit.sync="pages.pageSize"
        @pagination="handlePageChange" />
    </List>
    <!-- 新建GIS服务项目 -->
    <DialogsProject ref="DialogsProject" />
    <!-- 上传GIS数据 -->
    <DragUpload ref="DragUpload" :limit="null" accept=".dwg,.dxf,.svg" numType="uploadCADNum" @getFile="getFileDrag"
...
export default {
  components: { DragUpload, Pagination, DialogsProject, List, DialogShare },
  data() {
    return {
      loading: false,
      isList: true,
  ...
  methods: {
    ...
    toProject(row) {
      ...
      const url = `${config.CAD_Web}?file=${fileName}&token=f1962b88-87a5-495b-9848-b6630bb863bd`
      window.open(url)
    },
    share(row) {
      this.$refs.DialogShare.show(row)
    }
  },
};
</script>
