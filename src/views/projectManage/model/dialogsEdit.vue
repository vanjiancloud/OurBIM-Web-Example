...
<template>
  <el-dialog title="编辑项目" :visible.sync="dialogVisible" width="55%" :before-close="hide" :close-on-click-modal="false">
    <el-form :model="form" :rules="rules" ref="form" label-width="168px">
      ...
      <el-form-item label="项目GIS原点:" required v-if="form.isGis === 'true'">
        <el-col :span="7">
          <el-form-item prop="gisLongitude">
            <el-input v-model="form.gisLongitude" placeholder="经度"
        ...
        <el-col :span="7">
          <el-form-item prop="gisLatitude">
            <el-input v-model="form.gisLatitude" placeholder="纬度"
      ...
      <el-form-item :label="form.isGis === 'true' ? 'BIM模型:' : '子模型列表:'"
        v-if="form.appType === '3' || (form.appType === '0' && form.isGis === 'true')">
        <el-card class="box-card">
          <div v-for="(GISitem, index) in form.gisInfo" :key="index" class="flexStart">
            ...
            <el-button
              ...
              type="text" class="blueText" @click="delModel(index)">移除</el-button>
          </div>

          <!-- 链接模型展示 -->
          <div class="addNewModel" v-if="form.appType === '3'">
            <el-button v-if="!modelList.length" type="primary" @click="addModelClick()" plain
              class="bluePlainBtn1">添加模型</el-button>
      ...
      <el-form-item label="是否开启实时光线追踪:" v-if="form.appType !== '5'">
  ...
  methods: {
    show(row) {
      ...
      this.$nextTick(() => {
        ...
        // console.log('newGisInfo',newGisInfo);
    ...
    addModelClick(type = false) {
      let combineIds = this.form.gisInfo && this.form.gisInfo.map(e => e.appId) || []
      this.modelList = this.ListLinkModel.filter(item => combineIds ? !combineIds.includes(item.appid) : item)
    ...
    addGisServe(data) {
      ...
      let msgList = this.form.gisServerMsgList.length > 0 ? this.form.gisServerMsgList : []
      const realObj = this.GISModel.find(item => item.gisId === data)
      realObj ? msgList.push(realObj) : ''
      this.form.gisServerMsgList = msgList
      this.selectGisList = []
      this.form.selectGis = ''
    },
    ...
    deleteGisServe(key) {
      this.$confirm('确定移除吗', '提示').then(() => {
        ...
        let msgList = this.form.gisServerMsgList.length > 0 ? this.form.gisServerMsgList : []
        const realIndex = msgList.findIndex(item => item.gisId === key)
        realIndex !== -1 ? msgList.splice(realIndex, 1) : ''
        // console.log(msgList);
        this.form.gisServerMsgList = msgList
    ...
    submit() {
      this.$refs.form.validate((valid) => {
        ...
        // const data = {
        //     'gisServerMsgList': this.form.gisServerMsgList 
        // }
        const formData = new FormData();
        formData.append('gisServerMsgList', JSON.stringify(this.form.gisServerMsgList));
        delete params.gisServerMap
        delete params.sonAppMap
        delete params.gisServerMsgList
        // 转换成bim模型时 删掉链接gis服务数据的参数
        if (this.form.isGis == 'false') {
          // params.gisServerList = ''
          delete params.gisServerList
        }
        // console.log(params, this.form.gisServerMsgList)
        updateProject(params, formData).then(() => {
