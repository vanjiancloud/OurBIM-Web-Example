<template>
  <div class="box">
    ...
    <DialogsDrag ref="DialogsDrag" numType="uploadDoc" @getFile="getFileDrag" @onSuccess="getList"
...
import { deleteCloudDoc, getDocUrl, getUserAllCloudDoc, getCloudDocDownloadUrl } from '@/api/projectManage/officeManage.js'
export default {
  name: 'office',
  components: {
    DialogsDrag,
    ...
    downloadFile(item) {
      this.$confirm('即将下载此文件, 是否继续?', '提示', {
        ...
        .then(() => {
          ...
          getCloudDocDownloadUrl(params).then(res => {
            if (res?.message) {
