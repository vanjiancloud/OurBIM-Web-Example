<template>
  <List @change="onChange" style="padding: 10px 24px 20px 24px;" v-loading="loading">
    ...
    <el-table :data="tableData" @cell-click="cellClick" v-if="isList && isGroup" :row-style="{ 'cursor': 'pointer' }">
      ...
      <el-table-column prop="count" label="包含贴图数量" />
    ...
    <!-- 二级贴图 -->
    <el-table :data="tableData" @cell-click="showImage" v-if="isList && !isGroup"
      :header-cell-style="{ 'background': '#D0DCE3', 'color': '#333333' }" :row-style="{ 'cursor': 'pointer' }">
      <el-table-column prop="textureName" label="贴图名称" />
      <el-table-column prop="textureId" label="贴图ID" />
      <el-table-column prop="createTime" label="上传时间" />
      <el-table-column label="文件大小">
        <template slot-scope="scope">
          <!-- {{ (scope.row.fileLongSize / 1024).toFixed(0) }}KB -->
      ...
      <el-table-column fixed="right" label="操作" width="180">
        <template slot-scope="scope">
          <el-button type="text" class="blueText" @click.stop="download(scope.row)">下载</el-button>
    ...
    <div class="listBox" v-if="!isList">
      <div class="listItem" v-for="(item, index) in tableData" :key="index"
        @click="item.isGroup === '1' ? cellClick(item) : showImage(item)" :style="{ cursor: 'pointer' }">
        <el-image :src="item.isGroup === '1' ? item.imgPath : item.pngPath" class="defaultImg">
        ...
        <div class="flexBetween">
          <div>
            <el-button class="blueBtn" type="primary" size="small"
              @click.stop="item.isGroup === '1' ? add(item) : addChartlet(item)">编辑</el-button>
          </div>
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item @click.native="download(item)" v-if="item.isGroup != '1'">下载</el-dropdown-item>
    ...
    <!-- 新建、编辑贴图 -->
    <DialogCharlet ref="DialogCharlet" @reloadTable="getTextureList" />
    <!-- 图片预览组件 -->
...
export default {
  components: { List, DialogsChartletGroup, DialogCharlet, ImageViewer },
  props: {
    total: {
      type: Number,
      default: 0,
  ...
  methods: {
    ...
    showImage(item) {
      this.$refs.ImageViewerRef.show(item.imgPath)
    },
    download(item) {
      downloadImage(item.imgPath, item.textureName)
    },
    // 新增、编辑贴图
    addChartlet(row = {}) {
