  ...
  <List @change="onChange" style="padding: 10px 24px 20px 24px;" v-loading="loading">
    <template slot="title">
      <div class="boxHeader">
        ...
        <div class="boxHeaderTitle" v-if="!isGroup">当前分组有<span>{{ total }}</span>个材质文件</div>
        <div>
          ...
          <el-badge :value="uploadCom" :hidden="!uploadCom" v-if="!isGroup">
            <el-button class="blueBtn" type="primary" @click="addMeterial()">上传材质</el-button>
          </el-badge>
        </div>
      </div>
    </template>
    ...
    <el-table :data="tableData" @cell-click="cellClick" v-if="isList && isGroup" :row-style="{ 'cursor': 'pointer' }">
      ...
      <el-table-column prop="count" label="包含材质数量" />
      <el-table-column prop="isGroup" label="类型">
        <template slot-scope="scope">
          {{ scope.row.isGroup === '1' ? '材质分组' : '材质' }}
        </template>
      </el-table-column>
    ...
    <el-table :data="tableData" v-if="isList && !isGroup"
      ...
      <el-table-column prop="textureName" label="材质名称" />
      <el-table-column prop="textureId" label="材质ID" />
        ...
        <div class="flexBetween">
          <div>
            <el-button class="blueBtn" type="primary" size="small"
              @click.stop="item.isGroup === '1' ? add(item) : addMeterial(item)">编辑</el-button>
          </div>
          <el-dropdown>
    ...
    <!-- 新建/编辑分组 -->
    <DialogsMeterialGroup ref="DialogsMeterialGroup" />
    <!-- 新建/编辑材质 -->
    <DialogMeterial ref="DialogMeterial" :parentData="parentData" @reloadTable="getTextureList" />
  </List>
</template>
...
export default {
  components: { List, DialogsMeterialGroup, DialogMeterial },
  props: {
    total: {
      type: Number,
      default: 0,
  ...
  methods: {
    ...
    // 删除,删除构件
    deleteRow(row) {
      this.$confirm(`此操作将删除该【${this.isGroup ? row.groupName : row.textureName}】${this.isGroup ? '分组' : '材质'}, 是否继续?`, "提示", {
