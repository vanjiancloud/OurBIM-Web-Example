<template>
  <List @change="onChange" style="padding: 10px 24px 20px 24px;" v-loading="loading">
      ...
      <div class="boxHeader">
        ...
        <div class="header_row">
          <el-form :inline="true" :model="searchForm" style="display: inline-flex; align-items: center;"
            ...
            <el-form-item :label="isGroup ? '分组名称:' : '标签名称:'">
              <el-input v-model="searchForm.name" placeholder="请输入" style="flex: 1; min-width: 80px;"
                clearable></el-input>
            </el-form-item>
            <el-form-item :label="isGroup ? '分组ID:' : '标签ID:'">
              <el-input v-model="searchForm.tagId" placeholder="请输入" style="flex: 1; min-width: 80px;"
                clearable></el-input>
            </el-form-item>
            <el-form-item>
    ...
    <div class="listBox" v-if="!isList">
      <div class="listItem" v-for="(item, index) in tableData" :key="index" @click="isGroup ? cellClick(item) : ''"
        :style="{ cursor: isGroup ? 'pointer' : 'auto' }">
        ...
        <div class="flexBetween">
          <div>
            <el-button class="blueBtn" type="primary" size="small"
              @click.stop="isGroup ? add('编辑分组', item) : add('编辑标签', item)">编辑</el-button>
          </div>
          <el-dropdown>
    ...
    <Pagination v-if="total" :total="total" :page.sync="pages.pageNum" :limit.sync="pages.pageSize"
      @pagination="handlePageChange" />
    <!-- 新增编辑分组 -->
    <DialogTagGroup ref="DialogTagGroup" />
    <!-- 上传标签 -->
    <DialogsDrag ref="DialogsDrag" numType="tagUploadNum" accept=".png" @getFile="getFileDrag" @onSuccess="addTag">
    </DialogsDrag>
  </List>
</template>
  ...
  methods: {
    ...
    onSearch() {
      if (this.isGroup) {
        ...
        this.getListUserTagByGroupId(this.parentId)
      }
    },
    getList() {
    ...
    cellClick(row, reFresh = false) {
      ...
      this.getListUserTagByGroupId(row.tagId)
      this.parentId = row.tagId;
    },
    getListUserTagByGroupId(tagId) {
      let data = {
        ...this.searchForm,
        groupIds: tagId
      }
