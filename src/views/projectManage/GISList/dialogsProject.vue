        ...
        >
            <el-form :model="form" :rules="rules" ref="form" label-width="140px">
                ...
                <el-form-item
                    label="服务支持组件："
                    prop="gisPlugin"
                    v-if="form.layerType === 'OurGIS' || title === '添加'"
                >
                    <el-select v-model="form.gisPlugin" placeholder="请选择" @change="form.ourGISLayerList = []">
                        <el-option
                            :value="item.key"
                            :label="item.name"
                            v-for="(item, index) in gisPluginList"
                            :key="index"
                            :disabled="item.key === 'arcGIS'"
                        ></el-option>
                    </el-select>
                </el-form-item>
              ...
              <el-option :value="item.value" :label="item.note" v-for="(item,index) in gisCoordinateTypeList" :key="index"></el-option>
                    ...
                    <el-col :span="7">
                        <el-form-item prop="longitude">
                            <el-input
                                ...
                                placeholder="经度"
                                v-only-number="{ min: -180, max: 180, precision: 8 }"
                            ></el-input>
                        </el-form-item>
                    </el-col>
                    ...
                    <el-col :span="7">
                        <el-form-item prop="latitude">
                            <el-input
                                ...
                                placeholder="纬度"
                                v-only-number="{ min: -90, max: 90, precision: 8 }"
                            ></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col class="GISMark" :span="1">°</el-col>
                    ...
                    >
                        ...
                        <el-table-column>
                            <template slot-scope="scope">
                                <span v-if="scope.row.longitude"
                                    >{{ scope.row.longitude }}° {{ scope.row.latitude }}°
                                    {{ scope.row.altitude }}m</span
                                >
                                <span v-else>无</span>
                            </template>
                        </el-table-column>
                ...
                <el-form-item label="最大并发数：" v-if="title === '编辑'">
                    <el-input
                        ...
                        v-only-number="{ min: 0, precision: 0 }"
                        placeholder="请输入"
                    ></el-input>
                </el-form-item>
                <el-form-item label="鼠标操作模式：" v-if="title === '编辑'">
                    <el-select v-model="form.doMouse" placeholder="请选择" style="width:100%" v-if="title === '编辑'">
                        <el-option
                            :value="item.value"
                            :label="item.note"
                            v-for="(item, index) in doMouseList"
                            :key="index"
                        ></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="窗口显示模式：" v-if="title === '编辑'">
                    <el-select v-model="form.displayWindow" placeholder="请选择" style="width:100%">
                        <el-option
                            :value="item.value"
                            :label="item.note"
                            v-for="item in displayWindowList"
                            :key="item.value"
                        ></el-option>
                    </el-select>
                </el-form-item>
