 ...
 * @LastEditors: zk
 * @LastEditTime: 2024-10-20 19:57:34
 * @Description: 
-->
<!-- GIS数据服务 -->
<template>
  <List v-loading="loading" @change="onChange">
    ...
    <el-table :data="tableData" v-if="isList">
      ...
      <el-table-column prop="status" label="状态">
        <template slot-scope="scope">
          <span class="status"
            :class="{ 'status1': scope.row.status == 1, 'status3': scope.row.status == 3, 'status4': scope.row.status == 4, 'status5': scope.row.status == 5 }">{{
        ...
        <template slot-scope="scope">
          <div class="flexBetween">
            <el-button type="text" class="blueText" @click="toProject(scope.row)"
              :disabled="scope.row.status != 2">进入项目</el-button>
            <el-dropdown>
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-if="scope.row.status == 2"
                  @click.native="AddGISProgect('编辑', scope.row)">编辑</el-dropdown-item>
                ...
                <el-dropdown-item v-if="scope.row.status == 2" @click.native="onShare(scope.row)">分享</el-dropdown-item>
                <el-dropdown-item v-if="scope.row.status == 2"
                  @click.native="uploadRow(scope.row)">下载</el-dropdown-item>
                <el-dropdown-item :divided="scope.row.status == 2"
                  @click.native="deleteRow(scope.row)">删除</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </div>
        </template>
    ...
    <div class="listBox" v-if="!isList">
      <div class="listItem" v-for="(item, index) in tableData" :key="index">
        ...
        <div class="flexBetween">
          ...
          <div class="status"
            :class="{ 'status1': item.status == 1, 'status2': item.status == 2, 'status3': item.status == 3, 'status4': item.status == 4, 'status5': item.status == 5 }">
            {{ status[item.status] }}</div>
        </div>
        <div class="flexBetween">
          <div>
            <el-button class="blueBtn" type="primary" size="small" :disabled="item.status != 2"
          ...
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item v-if="item.status == 2" @click.native="onShare(item)">分享</el-dropdown-item>
    ...
    <!-- 上传GIS数据 -->
    <DialogsDrag ref="DialogsDrag" :limit="1" numType="uploadGISNum" @getFile="getFileDrag" @onSuccess="getList"
          ...
          <div v-if="form.layerType !== 'terrain' && form.layerType">
            <el-collapse v-model="activeName" accordion>
              <el-collapse-item title="GIS服务设置" name="1">
                ...
                <!-- WMTS -->
                <part-WMTS ref="refWMTS" v-if="form.layerType === 'WMTS'" />
                <!-- TMS -->
                <part-tms ref="refTms" v-if="form.layerType === 'TMS'" />
                <!-- WMS -->
...
export default {
  components: {
    DialogsProject, DialogsDrag, Pagination, List, PartTiles, PartWMTS,
    PartTms, PartWMS,
    PartShare
  },
  props: {},
  ...
  methods: {
    ...
    onShare(row) {
      console.log(row);
      this.$refs.refShare.show(row.gisId)
    },
    // 下载
    uploadRow(row) {
