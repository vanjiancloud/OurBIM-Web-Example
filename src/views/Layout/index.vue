<template>
  <el-container class="container">
    ...
    <el-header>
      <div class="head">
        <div class="headLeft">
          <a href="http://www.ourbim.com"><img class="logo-index" :src="require('@/assets/images/logo/logo_home.png')"
              alt="" /></a>
          <el-menu :default-active="$route.path" text-color="#333333" active-text-color="#FF6600" class="nav-menu"
            mode="horizontal">
            <template v-for="(item, index) in menuList">
              ...
              <el-submenu v-else :index="item.url" popper-class="popper-class" :key="index + 1">
                ...
                <el-menu-item :index="childItem.url" v-for="(childItem, childIndex) in item.children" :key="childIndex">
                  <a :href="childItem.url" class="aMenu" :target="childItem.blank ? '_blank' : ''">{{ childItem.name
                    }}</a>
                </el-menu-item>
              </el-submenu>
            </template>
    ...
    <el-main>
      ...
      <div class="statistics">
        ...
        <div class="statisticsBox server">
          ...
          <div class="flexSpaceBetween needFee" v-if="total.billingMode === '0'">
            ...
            <div class="serverItem">
              ...
              <div class="flexSpaceBetween needNum">
                ...
                <div>
                  <div class="needFeeNum">{{ total.useArConcurrency + total.useVrConcurrency + total.useMrConcurrency
                    }}<span>个</span></div>
                  <div class="needFeeTitle">
                    VR/AR/MR并发
                  </div>
                ...
                <div>
                  <div class="needFeeNum">{{ total.usePreConcurrency }}<span style="color: #6A7C87;font-size: 24px;">/{{
                    total.preConcurrency }}</span><span style="color: #6A7C87;">个</span></div>
                  <div class="needFeeTitle">
                    预启动并发
                  </div>
                </div>
          ...
          <div class="serverBox" v-else>
            <div class="serverItem">
              <div class="serverPer" :class="{ colorGray: !total.useStore }">{{
                total.useStore && (Number(total.useStore) / Number(total.store) * 100).toFixed(2) || 0 }}%</div>
              <el-progress :text-inside="true" :stroke-width="16"
                :percentage="Number(total.useStore / total.store * 100) || 0" :show-text="false"
                color="#02AAF0"></el-progress>
            ...
            <div class="serverItem">
              <div class="flexBetween">
                <div class="serverPer" :class="{ colorGray: !total.useConcurrency }">{{
                  total.useConcurrency && (total.useConcurrency / total.countConcurrency * 100).toFixed(2)||0 }}%</div>
                <div class="serverTotal">总并发 {{ total.useConcurrency }}/{{ total.countConcurrency }}</div>
              </div>
              <div id="erupt"></div>
              <div class="serverUsed">
                <img src="./img/jiedian.png" alt="" />
                <span>云VR/AR/MR {{ total.useArConcurrency + total.useVrConcurrency + total.useMrConcurrency }}/{{
                  total.vrConcurrency }}
        ...
        <div class="statisticsBox account">
          ...
          <div class="accountBox">
            <div class="accountItem" style="text-align: right;">
              <div class="text">账户余额：<span :class="{ colorGray: !total.money }">{{ total.money || 0
                  }}</span><span>资源点</span>
              </div>
              <el-button type="primary" size="small" class="orangeBtn"
                style="border:0;width: 120px;background-color:#FF7F28"
                @click="$router.push('/expense/recharge')">立即充值</el-button>
            </div>
            <div class="accountItem" style="min-width:195px;">
              <div class="text">授权码：<span style="color:#00AAF0" :class="{ colorGray: !total.codeSum }">{{ total.codeSum ||
                  0
      ...
      <div class="wrap">
        ...
        <div class="nav" :class="fold ? 'nav' : 'closeNav'">
          ...
          <el-menu :collapse="!fold" :default-active="activeMenu" :collapse-transition="false" class="tac"
            text-color="#999999" active-text-color="#ffffff" router background-color="#fff">
