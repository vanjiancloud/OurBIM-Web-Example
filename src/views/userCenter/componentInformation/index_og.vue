      ...
      <div class="materialList" v-if="componentAllInfo.matList && componentAllInfo.matList.length">
        <div class="materialListCon" :style="{ 'height': isOpen ? 'auto' : '90px' }">
          <div class="materialItem" :class="{ activeMaterial: activeMaterialIndex === index }"
            ...
            <div class="materialReset" @click.stop="resetMaterial(item, index)"
              v-if="materialAllInfo.matParam && ((materialAllInfo.matParam.baseParamsList && materialAllInfo.matParam.baseParamsList.length) || (materialAllInfo.matParam.colorList && materialAllInfo.matParam.colorList.length)) && activeMaterialIndex === index">
      ...
      <template v-if="materialAllInfo.matParam">
        ...
        <template v-if="materialChartlet.baseParamsList && materialChartlet.baseParamsList.length">
          ...
          <template v-for="(item, index) in materialChartlet.baseParamsList">
            <div class="materialSlider" :key="index + 'base'"
              v-if="!item.hasOwnProperty('enableEdit') || item.enableEdit != 'false'">
              <div>{{ item.label }}</div>
              <div class="slider">
                <el-slider @change="onChange(1, $event, index)" v-model="item.paramValue1" :max="Number(item.max)"
                  :min="Number(item.min)"
        ...
        <div>
          ...
          <el-checkbox v-if="componentAllInfo.matList.length" v-model="allMaterialChecked"
            @change="changeAllMaterialChecked">替换所有相同材质</el-checkbox>
        </div>
      </template>
    </div>
  </Drawer>
...
export default {
  ...
  data() {
    return {
      ...
      allMaterialChecked: false,//是否全部替换材质
    }
  },
  watch: {
  ...
  created() {
    ...
    this.$store.watch((state) => state.material.materialAllInfo, (newValue, oldValue) => {
      ...
      this.materialChartlet.colorList = this.formatBaseParams(newValue.matParam.colorList)
      this.materialChartlet.textureParamsList = this.formatBaseParams(this.getChartletParams())
      this.materialChartlet.texturesList = this.formatBaseParams(newValue.matParam.texturesList)
  ...
  methods: {
    ...
    */
    ...
    onMaterial(item, i) {
      ...
      this.getMaterialInfo(item.matId)
    },
    // 获取材质信息
    ...
    resetMaterial(item, i) {
      this.$confirm('您将重置此材质, 是否继续?', '提示', {
        ...
        let params = {
          ...
          matId: item.matId,
          isPublic: false,
          isBatch: this.allMaterialChecked
        }
        let arr = [
          {
    ...
    // 是否替换全部材质
    changeAllMaterialChecked() {
      this.$store.dispatch('material/changeSetting', { key: "allMaterialChecked", value: this.allMaterialChecked })
    }
  }
}
</script>
