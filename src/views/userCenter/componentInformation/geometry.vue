...
export default {
  ...
  data() {
    return {
      ...
      geometryObjForm: {
        ...
        intensity: 10,//灯光强度
        radiationAngleOfInner: 30,//内辐射角
        radiationAngleOfOuter: 50,//外辐射角
        attenuationRadius: 250,//衰减半径
        sourceRadius: 1,//光源半径
        sourceLength: 0,//光源长度
        influenceRadius: 10,//影响半径
        brightness: 0.5,//光源亮度,反射强度
        sourceHeight: 10,//光源高度
        sourceWidth: 100,//光源宽度
        reflectionSourceType: 'CapturedScene',//反射源类型
        barnDoorAngle: 0,//谷仓门角度
        barnDoorLength: 0,//谷仓门长度
        // 表单的
        intensity1: 10,
        radiationAngleOfInner1: 30,
        radiationAngleOfOuter1: 50,
        attenuationRadius1: 250,
        sourceRadius1: 1,
        sourceLength1: 0,
        influenceRadius1: 10,
        brightness1: 0.5,
  ...
  destroyed() {
    // EventBus.$off('onBimMessage', this.handleGeometryBus)
  },
  methods: {
    setBus() {
    ...
    handleVideo(a) {
      if (a == 'play') {
        this.video.videoPlay = true
      }
      if (a == 'pause') {
        this.video.videoPlay = false
      }
    },
    videoUrlSubmit() {
      let params = {
      ...
      sendvideoUrl(params).then(res => {
        this.$message.success(res.message)
      })
    },
    getServerName(e) {
      if (!e.uuid) return
      const realObj = this.dataLayer.find(item => e.uuid === item.uuid)
      return realObj.gisServerName
    },
    deleteRow(e) {
      const { uuid } = e
      let realList = this.dataPolygon.filter(item => item.uuid !== uuid)
      realList = realList.filter(item => item.uuid)
      let realData = [...realList, ...[{
        uuid: null,
        materialLayer: []
      }]]

      this.dataPolygon = realData
      this.changeLayer()
    },
    // 删除构件
    deleteCom(uuid) {
      this.$confirm(`此操作将删除图元, 是否继续?`, "提示", {
    ...
    delCom(uuid) {
      delEle({
        taskId: this.data.taskId,
        uuid: uuid ? uuid : this.geometryObjForm.id,
      }).then((res) => {
        this.$message.success('删除成功')
        this.geometryObjForm.id = '';
    ...
    changeLayer() {
      ...
      let realList = this.dataPolygon.filter(item => item.uuid)
      const data = [{
        "polygonInfo": {
          "uuid": this.geometryObjForm.id,
          pointList: this.geometryObjForm.polygonInfo
        },
        "detail3DTileInfo": realList
      }]
      updateLayer(params, data).then(() => {
        let realData = [...realList, ...[{
          uuid: null,
          materialLayer: []
        }]]
    ...
    getLayerList() {
      ...
      listLayer(params).then(res => {
        this.dataLayer = res.data
      })
    },
    handleGeometryBus(data) {
      const { id, ourbimGisPolygon } = data
      if (id === '110') {
        // console.log('onBimMessage-data-110-几何信息',data)
        const { location, rotation, scale, uuid, polygonInfo, detail3DTileInfo } = ourbimGisPolygon
        // 坐标;比例,缩放
        let realLocation = location.split(' ')
        this.geometryObjForm.location = {
          x: realLocation[0].split('=')[1],
          y: realLocation[1].split('=')[1],
          z: realLocation[2].split('=')[1]
        }
        let realScale = scale.split(' ')
        this.geometryObjForm.scale = {
          x: realScale[0].split('=')[1],
          y: realScale[1].split('=')[1],
          z: realScale[2].split('=')[1]
        }
        // 角度
        let realRotation = rotation.split(' ')
        this.geometryObjForm.rotation = {
          p: realRotation[0].split('=')[1],
        ...
        if (detail3DTileInfo) {
          let realInfo = JSON.parse(detail3DTileInfo)
          let realList = realInfo.filter(item => item.uuid)
          let realData = [...realList, ...[{
            uuid: null,
            materialLayer: []
          }]]
    ...
    editTuyuan() {
      ...
      const { location, rotation, scale } = this.geometryObjForm
      const realLocation = `X=${location.x} Y=${location.y} Z=${location.z}`
      const realRotation = `P=${rotation.p} Y=${rotation.y} R=${rotation.r}`
      const realScale = `X=${scale.x} Y=${scale.y} Z=${scale.z}`
      let data = {
    ...
    // 水源编辑
    editSplashDam() {
      // console.log(this.splashDamForm)
      let data = {
        ...this.splashDamForm,
        type: 2
      }
