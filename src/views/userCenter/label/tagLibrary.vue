...
<template>
  <Drawer ref="Drawer" title="标签批注素材库" direction="ltr" @onClose="close">
    ...
    <div class="level2" v-if="levels.level === 2">
      <span @click="back" class="backLevel1"><i class="el-icon-arrow-left"></i>{{ levels.groupName }}</span> / <span
        class="level2Item">{{ levels.tabName }}</span>
    </div>
    <!-- 搜索 -->
    <div class="search">
      <el-input v-model="searchParam.name" size="mini" placeholder="请输入您要搜索的内容" prefix-icon="el-icon-search">
      </el-input>
      <el-button v-if="levels.tab1Index === 1" class="button blueBtn1" type="primary" icon="el-icon-plus" size="mini"
        @click="createGroup(levels.level === 1 ? '新建分组' : '新建标签样板')">{{ levels.level === 1 ? '新建分组' : '新建'
        }}</el-button>
    </div>
    <!-- 内容 -->
    <div class="content">
  ...
  methods: {
    ...
    toLevel2(item) {
      ...
      if (this.levels.level === 2) {
        ...
        if (['default', 'anchorCustomize'].includes(item.type)) {
          data = {
            ...data,
            bgRelateAnchor: item?.tagInfo?.bgRelateAnchor || '',
            bgRelateAlign: item?.tagInfo?.bgRelateAlign || '',
            bgRelateOffset: item?.tagInfo?.bgRelateOffset || "x=0,y=0",
          }
        }
