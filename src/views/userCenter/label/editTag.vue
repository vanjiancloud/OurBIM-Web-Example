<template>
  <el-form class="editTag" ref="form" :model="form" label-width="auto" v-if="data.clickTagData">
    ...
    <el-form-item label="标签样式" prop="type">
      <el-cascader ref="refCascader" popper-class="styleCascader" v-model="form.changeType" :options="newTagtypeList"
        :props="{ expandTrigger: 'hover', label: 'name', value: 'key' }" @change="changeType"></el-cascader>
    </el-form-item>
    <el-form-item prop="type" label-width="0">
      <div class="coordinate">
        ...
        <div class="coordinateItemInput">
          X<el-input-number v-model="form.location.x" controls-position="right" size="mini" @keydown.native.stop
            @change="edit()" />
          Y<el-input-number v-model="form.location.y" controls-position="right" size="mini" @keydown.native.stop
            @change="edit()" />
          Z<el-input-number v-model="form.location.z" controls-position="right" size="mini" @keydown.native.stop
            @change="edit()" />
        ...
        <template v-if="['customizeInWorld', 'webui3d'].includes(form.type)">
          ...
          <div class="coordinateItemInput">
            P<el-input-number v-model="form.rotation.p" controls-position="right" size="mini" @keydown.native.stop
              @change="edit()" />
            Y<el-input-number v-model="form.rotation.y" controls-position="right" size="mini" @keydown.native.stop
              @change="edit()" />
            R<el-input-number v-model="form.rotation.r" controls-position="right" size="mini" @keydown.native.stop
              @change="edit()" />
            <i class="el-icon-refresh-right"></i>
          </div>
          <div class="TagTitle" style="margin: 0 10px 0 10px;">比例：</div>
          <div class="coordinateItemInput">
            X<el-input-number v-model="form.scale.x" controls-position="right" size="mini" @keydown.native.stop
              @change="edit()" />
            Y<el-input-number v-model="form.scale.y" controls-position="right" size="mini" @keydown.native.stop
              @change="edit()" />
            Z<el-input-number v-model="form.scale.z" controls-position="right" size="mini" @keydown.native.stop
              @change="edit()" />
            <i class="el-icon-refresh-right"></i>
          </div>
        </template>
    ...
    <template v-if="['default', 'anchorCustomize'].includes(form.type)">
      <el-form-item prop="bgRelateAnchor" label="相对位置">
        <el-select v-model="form.bgRelateAnchor" placeholder="请选择" style="width:100%" @change="edit()">
      ...
      <el-form-item prop="bgRelateAlign" label="对齐方式">
        <el-select v-model="form.bgRelateAlign" placeholder="请选择" style="width:100%" @change="edit()">
      ...
      <el-form-item prop="bgRelateOffset" label="偏移量">
        <span class="TagTitle">x</span><el-input-number v-model="form.bgRelateOffset.x" :step="1" :controls="false"
          style="width: 30%;margin-left: 8px;" @change="edit()"></el-input-number>
    ...
    <template v-if="form.type === 'default'">
      ...
      <el-form-item label-width="0">
        ...
        <el-input v-model="form.iconSize.width" @keydown.native.stop placeholder="宽度(px)"
          style="width: 30%;;margin-left: 8px;" @change="edit()"></el-input>
        <span class="unit">px</span>
        <span class="unit">高</span>
        <el-input v-model="form.iconSize.height" @keydown.native.stop placeholder="高度(px)"
          style="width: 30%;margin-left: 8px;" @change="edit()"></el-input>
        <span class="unit">px</span>
      </el-form-item>
    ...
    <template v-else-if="['customize', 'anchorCustomize', 'customizeInWorld'].includes(form.type)">
      <el-form-item v-if="form.type === 'anchorCustomize'" label="锚点图标">
        <SingleUpload v-model="form.tagUrl" url="/tagControl/uploadTagImg" accept="image/png" @success="uploadSuccess">
        </SingleUpload>
      </el-form-item>
      ...
      <el-form-item v-if="form.type === 'customizeInWorld'" label="面板发光强度" label-width="130px">
        <el-input v-model="form.intensityOfEmissiveColor" @keydown.native.stop v-only-number="{ precision: 1 }"
        ...
        <el-table-column align="center" v-for="(item, index) in form.slotNum.column" :key="index">
          <template slot-scope="scope">
            <el-input :class="{ focusInput: focusInput.rowIndex === scope.$index && focusInput.colIndex === index }"
              @focus="focusTableItem(scope.row, { rowIndex: scope.$index, colIndex: index })"
              @change="changeTableName(scope.row)" v-model="scope.row[`text${index + 1}`]"></el-input>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-form-item label-width="0" v-if="form.type === 'customizeInWorld'">
        <el-checkbox v-model="form.camerafollow" true-label="true" false-label="false"
          @change="edit()">是否跟随摄像头转动</el-checkbox>
      </el-form-item>
    </template>
...
import { TagtypeList, fontType, bgRelateAnchorList, bgRelateAlignList } from './json'
...
export default {
  ...
  methods: {
    getData(val) {
      ...
      if (val.bgRelateOffset) {
        if (typeof val.bgRelateOffset == 'string') {
          val.bgRelateOffset = { x: val.bgRelateOffset.split(',')[0].split('x=')[1], y: val.bgRelateOffset.split(',')[1].split('y=')[1] }
        }
      }
      // 
      if (val.type === 'anchor') {
    ...
    edit() {
      ...
      if (['default', 'anchorCustomize'].includes(type)) {
        data = {
          ...data,
          bgRelateAnchor: this.form.bgRelateAnchor,
          bgRelateAlign: this.form.bgRelateAlign,
          bgRelateOffset: `x=${this.form.bgRelateOffset.x},y=${this.form.bgRelateOffset.y}`,
        }
      }
