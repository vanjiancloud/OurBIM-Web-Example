<template>
  <Drawer ref="Drawer" title="标签" direction="rtl" @onClose="close" class="tag-tree">
    <!-- 搜索 -->
    <div class="search">
      <el-input v-model="searchName" @click.native.stop @change="search" @keyup.enter="search" size="mini"
        placeholder="请输入您要搜索的内容" prefix-icon="el-icon-search"></el-input>
    ...
    <div class="tree-content">
      <el-tree ref="refTag" class="tree" :class="{ treeHeight: data.clickTagData }" :data="DataTagTree" :props="propsTag"
        :expand-on-click-node="false" :highlight-current="true" :default-expanded-keys="expandedKeys" empty-text="暂无数据"
        :filter-node-method="filterNode" node-key="id" :load="loadTag" :show-checkbox="true" @node-expand="nodeExpand"
        @node-collapse="nodeCollapse" @check="isShowTag" lazy>
        <div class="tag-slot" :class="{ 'tree-select': activeTree && data.id === activeTree.id }"
          slot-scope="{ node, data }" @click="handleTag(data)">
          <div class="name">
            <SingleUpload v-model="data.tagUrl" :autoUpload="false" :deleteIcon="false" :showImg="false"
              accept="image/png" @onChangeFile="onChangeFile($event, data)"></SingleUpload>
