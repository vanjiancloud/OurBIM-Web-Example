<!-- 新建/编辑标签 -->
<template>
  <el-dialog :title="title" :visible.sync="dialogVisible" :close-on-click-modal="false" append-to-body width="50%"
    :before-close="hide">
    <el-form ref="form" :style="{ 'width': '100%' }" :model="form" :rules="rules" label-width="auto">
      ...
      <el-form-item label="缩略图:">
        <SingleUpload v-model="form.tagUrl" url="/tagControl/uploadTagImg" accept="image/png" @success="uploadSuccess">
        </SingleUpload>
      </el-form-item>
      ...
      <el-form-item v-if="!['customize', 'customizeInWorld'].includes(form.type)"
        :label="['webui', 'webui3d'].includes(form.type) ? 'WebUI面板尺寸:' : '图标尺寸:'">
        <div class="flex">
          <span class="TagTitle">宽</span>
          <el-input v-model="form.iconSize.width" v-only-number="{ min: 0 }" @keydown.native.stop placeholder="宽度(px)"
            style="flex: 1;">
            <template slot="append">px</template>
          </el-input>
          <span class="TagTitle">高</span>
          <el-input v-model="form.iconSize.height" v-only-number="{ min: 0 }" @keydown.native.stop placeholder="高度(px)"
            style="flex: 1;">
            <template slot="append">px</template>
          </el-input>
        </div>
      </el-form-item>
      <!-- 普通标签和自定义锚点标签 支持设置标签名称/数据面板与标签图标的位置关系 -->
      <template v-if="['default', 'anchorCustomize'].includes(form.type)">
        <el-form-item prop="bgRelateAnchor" :label="form.type === 'default' ? '名称相对图标的位置:' : '面板相对图标的位置:'">
          <el-select v-model="form.bgRelateAnchor" placeholder="请选择" style="width:100%"
            @change="form.bgRelateAlign = undefined">
            <el-option v-for="(item, index) in bgRelateAnchorList" :key="index" :label="item.name" :value="item.key">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="bgRelateAlign" label="对齐方式:">
          <el-select v-model="form.bgRelateAlign" placeholder="请选择" style="width:100%">
            <el-option v-for="(item, index) in bgRelateAlignList.filter((i) => i.belong.includes(form.bgRelateAnchor))"
              :key="index" :label="item.name" :value="item.key">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="bgRelateOffset" :label="form.type === 'default' ? '名称相对图标的偏移:' : '面板相对图标的偏移:'">
          <div class="flex">
            <span class="TagTitle">x</span><el-input-number v-model="form.bgRelateOffset.x" :step="1" :controls="false"
              style="flex: 1;"></el-input-number>
            <span class="TagTitle">y</span><el-input-number v-model="form.bgRelateOffset.y" :step="1" :controls="false"
              style="flex: 1;"></el-input-number>
          </div>
        </el-form-item>
      </template>
      <template v-if="['anchorCustomize', 'customize', 'customizeInWorld'].includes(form.type)">
        <el-form-item prop="iconPath" label="数据面板背景图片:">
          <SingleUpload v-model="form.iconPath" url="/tagControl/uploadTagImg" accept="image/png"
            @success="uploadSuccessBg"></SingleUpload>
        </el-form-item>
        <el-form-item label="数据面板尺寸:">
          <div class="flex">
            <span class="TagTitle">宽</span>
            <el-input v-model="form.labelSize.width" v-only-number="{ min: 0 }" @keydown.native.stop
              placeholder="宽度(px)" style="flex: 1;">
              <template slot="append">px</template>
            </el-input>
            <span class="TagTitle">高</span>
            <el-input v-model="form.labelSize.height" v-only-number="{ min: 0 }" @keydown.native.stop
              placeholder="高度(px)" style="flex: 1;">
              <template slot="append">px</template>
            </el-input>
          </div>
        </el-form-item>
        <el-form-item prop="slotNum" label="数据面板表格:">
          <div class="flex">
            ...
            <el-input @change="changeTableSize" v-model="form.slotNum.row"
              v-only-number="{ max: 100, min: 0, precision: 0 }" @keydown.native.stop placeholder="行数" style="flex: 1;">
              <template slot="append">行</template>
            </el-input>
            <span class="TagTitle">列数</span>
            <el-input @change="changeTableSize" v-model="form.slotNum.column"
              v-only-number="{ max: 100, min: 0, precision: 0 }" @keydown.native.stop placeholder="列数" style="flex: 1;">
              <template slot="append">列</template>
            </el-input>
          </div>
        </el-form-item>
        <el-form-item label="数据表信息:">
          <el-table class="table" :data="form.tableStyle" size="mini" :show-header="false"
            ...
            <el-table-column align="center" v-for="(item, index) in form.slotNum.column" :key="index">
              <template slot-scope="scope">
                <el-input :class="{ focusInput: focusInput.rowIndex === scope.$index && focusInput.colIndex === index }"
                  @focus="focusTableItem(scope.row.slotList, { rowIndex: scope.$index, colIndex: index })"
                  v-model="scope.row[`text${index + 1}`]"></el-input>
              </template>
            </el-table-column>
          </el-table>
        </el-form-item>
        <template v-if="focusInput.rowIndex > -1">
          ...
          <el-form-item label="单元格信息"></el-form-item>
          <el-form-item label="单元格尺寸:">
            <div class="flex">
              <span class="TagTitle">长</span>
              <el-input v-model="styleForm.size.width" v-only-number="{ min: 0 }" @keydown.native.stop
                placeholder="宽度(px)" style="flex: 1;">
                <template slot="append">px</template>
              </el-input>
              <span class="TagTitle">宽</span>
              <el-input v-model="styleForm.size.height" v-only-number="{ min: 0 }" @keydown.native.stop
                placeholder="高度(px)" style="flex: 1;">
                <template slot="append">px</template>
              </el-input>
            </div>
          ...
          <el-form-item label="字体大小:">
            <el-input v-model="styleForm.fontSize" @keydown.native.stop
              v-only-number="{ min: 10, precision: 0 }"></el-input>
          </el-form-item>
          ...
          <el-form-item label="文字内边距:">
            <div class="flex margin">
              <span class="marginTitle">左</span>
              <el-input class="marginInput" v-model="styleForm.margin[0]" v-only-number="{ min: 0, precision: 0 }"
                @keydown.native.stop placeholder="左">
                <template slot="append">px</template>
              </el-input>
              <span class="marginTitle">上</span>
              <el-input class="marginInput" v-model="styleForm.margin[1]" v-only-number="{ min: 0, precision: 0 }"
                @keydown.native.stop placeholder="上">
                <template slot="append">px</template>
              </el-input>
              <span class="marginTitle">右</span>
              <el-input class="marginInput" v-model="styleForm.margin[2]" v-only-number="{ min: 0, precision: 0 }"
                @keydown.native.stop placeholder="右">
                <template slot="append">px</template>
              </el-input>
              <span class="marginTitle">下</span>
              <el-input class="marginInput" v-model="styleForm.margin[3]" v-only-number="{ min: 0, precision: 0 }"
                @keydown.native.stop placeholder="下">
                <template slot="append">px</template>
              </el-input>
            </div>
          </el-form-item>
          <el-form-item label="文字外边距:">
            <div class="flex margin">
              <span class="marginTitle">左</span>
              <el-input class="marginInput" v-model="styleForm.padding[0]" v-only-number="{ min: 0, precision: 0 }"
                @keydown.native.stop placeholder="左">
                <template slot="append">px</template>
              </el-input>
              <span class="marginTitle">上</span>
              <el-input class="marginInput" v-model="styleForm.padding[1]" v-only-number="{ min: 0, precision: 0 }"
                @keydown.native.stop placeholder="上">
                <template slot="append">px</template>
              </el-input>
              <span class="marginTitle">右</span>
              <el-input class="marginInput" v-model="styleForm.padding[2]" v-only-number="{ min: 0, precision: 0 }"
                @keydown.native.stop placeholder="右">
                <template slot="append">px</template>
              </el-input>
              <span class="marginTitle">下</span>
              <el-input class="marginInput" v-model="styleForm.padding[3]" v-only-number="{ min: 0, precision: 0 }"
                @keydown.native.stop placeholder="下">
                <template slot="append">px</template>
              </el-input>
            </div>
          ...
          <el-form-item label="对齐方式:">
            <div class="aligning">
              <span @click="styleForm.horAlign = item.key" :class="{ activeAlign: styleForm.horAlign === item.key }"
                v-for="(item, index) in horAlignList" :key="index">
          ...
          <el-form-item label="垂直对齐:">
            <div class="aligning">
              <span @click="styleForm.verAlign = item.key" :class="{ activeAlign: styleForm.verAlign === item.key }"
                v-for="(item, index) in vertAlignList" :key="index">
                <svg-icon :icon-class="item.name" class="svgBtn" />
              </span>
            </div>
          </el-form-item>
...
import { typeList, fontType, horAlignList, vertAlignList, TagtypeList, bgRelateAnchorList, bgRelateAlignList } from './json'
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        tableStyle: [],
        tagInfo: {},
        bgRelateAnchor: 'right',
        bgRelateAlign: 'center',
        bgRelateOffset: {
          x: 0,
          y: 0
        }
  ...
  methods: {
      ...
      this.$nextTick(() => {
        ...
        if (row.tagId) {
          ...
          if (row.tagInfo) {
            ...
            if (row.tagInfo.bgRelateAnchor) {
              this.$set(this.form, 'bgRelateAnchor', row.tagInfo.bgRelateAnchor)
            }
            if (row.tagInfo.bgRelateAlign) {
              this.$set(this.form, 'bgRelateAlign', row.tagInfo.bgRelateAlign)
            }
            if (row.tagInfo.bgRelateOffset) {
              this.$set(this.form, 'bgRelateOffset', { x: row.tagInfo.bgRelateOffset.split(',')[0].split('x=')[1], y: row.tagInfo.bgRelateOffset.split(',')[1].split('y=')[1] })
            }
          }
        }
        if (row.parentId) {
    ...
    submit() {
      this.$refs.form.validate((valid) => {
        ...
        let { type, name, parentId, tagId, tagInfo, iconSize, bgRelateAnchor, bgRelateAlign, bgRelateOffset } = this.form
        let data = {
          type,
          name,
          parentId,
        ...
        // 普通标签和自定义锚点标签的偏移量
        if (['default', 'anchorCustomize'].includes(type)) {
          data.tagInfo.bgRelateAnchor = bgRelateAnchor;
          data.tagInfo.bgRelateAlign = bgRelateAlign;
          data.tagInfo.bgRelateOffset = `x=${bgRelateOffset.x},y=${bgRelateOffset.y}`
        }
        // 
        if (this.form.tagId) {
