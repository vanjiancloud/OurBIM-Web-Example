...
const frame2Time = (frame, fps) => {
  ...
  return frame % fps === 0 ? second2hms(frame / fps) : (frame % fps) + "f";
}
...
const frameWidth2Grid = frameWidth => {
  ...
  // console.log('tempGridSegment', tempGridSegment)
  if (!tempGridSegment) {
...
const getGridBufferNumber = (timeLine_width, gridWidth) => {
  // console.log(timeLine_width, gridWidth,timeLine_width/ gridWidth)
  return Math.ceil(timeLine_width / gridWidth) + 1;
}

/**
 * 渲染格子
 * @param {ref<List>} gridBufferList 最终要渲染的 GridBufferList
 * @param {*} gridWidth 每个格子的宽度 15
 * @param {number} groupGridFrame 每个 GridGroup 里包含多少 Frame 300
 * @param {number} gridFrame 每个 Grid 里包含多少 Frame 30
 * @param {width} timeLineOffsetLeft 时间轴的左偏移量 0
 * @param {*} timescale_width 时间轴的实际宽度（不是显示宽度）810
 */
const renderGridBufferList = (
  ...
  // console.log('渲染grid', gridBufferList,
  //   gridWidth,
  //   groupGridFrame,
  //   gridFrame,
  //   timeLineOffsetLeft,
  ...
  try {
    ...
    // 结束位置
    const gridBufferNumber = getGridBufferNumber(timeLine_width, gridWidth) - 2 ;
    // 动态计算数组长度
    if (gridBufferNumber > gridBufferList.length) {
      const dValue = gridBufferNumber - gridBufferList.length;
    ...
    } else if (gridBufferNumber < gridBufferList.length) {
      const dValue = gridBufferList.length - gridBufferNumber;
      gridBufferList.splice(gridBufferList.length - dValue, dValue);
    } else {
      // doing nothing
    }
    ...
    for (let i = firstIndex; i <= gridBufferNumber + firstIndex - 1; i++) {
      const grid = gridBufferList[i - firstIndex];
      if (i - 1 === 0) {
        grid.frame = 0;
...
const getMaxFrameOfMaterial = (maxData) => {

  return maxData === 0
    ? 0
    : μs2Frame(maxData * 1000, 30);
};
