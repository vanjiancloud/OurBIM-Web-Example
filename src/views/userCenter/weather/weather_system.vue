            ...
            <div class="templateBox">
              ...
              <div class="templateInfo" v-if="radio === 0">
                <el-select v-model="valueTemplate" placeholder="请选择" size="mini" @change="valueChangeBtn">
                  <el-option :key="index" v-for="(item, index) in lessOptions" :label="item.weatherName"
                    :value="item.id">
                  </el-option>
                </el-select>
              </div>
            ...
            <div class="boxSolidBackground">
              ...
              <div class="colorBox" v-if="radio === 1">
                <div class="selectColor">
                  <el-color-picker :disabled="radio == 1 ? false : true" v-model="color1" size="mini" color-format="rgb"
                    @change="changeColor"></el-color-picker>
                </div>
                <div class="colorName">
                  <el-input v-model="input" size="mini" @blur="inputBlur"
                    :disabled="radio == 1 ? false : true"></el-input>
                </div>
              </div>
      ...
      <template v-if="fillLight.affectWorld">
        <div class="fillLightItem">
          ...
          <div class="fillLightItem flexBetween">
            <el-slider v-model="fillLight.strengthSlider" :min="0" :max="1" :step="0.1" class="slider"
              @change="changeFillLightSlider"></el-slider>
            <el-input v-model="fillLight.strength" size="small" class="input" @keydown.native.stop
              @change="changeFillLight" v-only-number="{ min: 0, precision: 2 }"></el-input>
            <span>Lux</span>
          </div>
        </div>
        <el-checkbox style="margin-left: 20px;" v-model="fillLight.castShadows" label="投射阴影"
          @change="changeFillLight" />
      </template>
    </div>
  </div>
</template>
