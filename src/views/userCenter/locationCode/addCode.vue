...
<template>
  <el-form class="addCode" ref="form" :style="{ 'width': '95%' }" :model="form" :rules="rules" label-width="100px">
    ...
    <template v-if="anchorPoint || data.type === 2">
      <el-form-item>
        <el-button v-if="!updateCode && data.type !== 2" class="blueBtn1" type="primary" size="mini"
          @click="updateLocator()">更新定位点</el-button>
        <el-button v-if="updateCode" class="blueBtn" type="primary" size="mini" @click="confirmUpdateLocator(true)">确
          定</el-button>
        <el-button v-if="updateCode" class="grayPlain" type="primary" size="mini" @click="confirmUpdateLocator(false)">恢
          复</el-button>
      </el-form-item>
      <el-form-item label="视 图：">
        <el-image class="codeViewImg" :src="form.codeViewImg" fit="fit"
          @click="focusCode({ codeIds: form.codeId, focus: true })"></el-image>
      </el-form-item>
      ...
      <el-form-item label="定 位 码 ：">
        <el-image class="qrCode" :src="form.qrCode" fit="fit"
          @click="focusCode({ codeIds: form.codeId, focus: true })"></el-image>
      </el-form-item>
    </template>
    <el-form-item>
      <el-button v-if="!surePlace && !anchorPoint && data.type !== 2" class="blueBtn" type="primary" size="small"
        @click="addCode()">选择定位点</el-button>
      <el-button v-if="surePlace" class="blueBtn" type="primary" size="mini" @click="sureLocator(true)">确 定</el-button>
      <el-button v-if="surePlace" class="grayPlain" type="primary" size="mini" @click="sureLocator(false)">取
        消</el-button>
      <el-button v-if="anchorPoint || data.type === 2" class="blueBtn" type="primary" size="small" @click="downCode()">下
        载</el-button>
    </el-form-item>
  </el-form>
</template>
