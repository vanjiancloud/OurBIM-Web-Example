    ...
    <Drawer ref="Drawer" :title="title" @onClose="close">
      <template v-slot="{ isShow }">
        ...
        <div class="view_photo" v-if="viewPic === '1'">
          ...
          <div class="search">
            <el-input @blur="inputBlur" class="searchInput" type="text" v-model="input" @keyup.enter.native="searchBtn"
              placeholder="请输入视图名称">
            ...
            <div class="threeLogo">
              <span>
                <el-tooltip class="item" effect="light" content="更新视图" placement="top">
                  <i class="iconfont icon-a-gengxinshitu3" @click="coverClick" :class="{ bind: active === -1 }"
                    :style="{ 'font-size': '24px' }"></i>
                </el-tooltip>
              </span>
              <span :style="{ 'margin-bottom': '1px' }">
                <el-tooltip class="item" effect="light" content="导出截图" placement="top">
                  <i class="iconfont icon-a-gengxinshitu2" @click="exportPic" :class="{ bind: active === -1 }"
                    :style="{ 'font-size': '24px' }"></i>
                </el-tooltip>
              </span>
              <span :style="{ 'margin-top': '2px' }">
                <el-tooltip class="item" effect="light" content="创建视图" placement="top">
                  <i class="iconfont icon-a-gengxinshitu1" :class="{ bind: active !== -1 }"
                    :style="{ 'font-size': '22px' }" @click="InsertFollow"></i>
                </el-tooltip>
              </span>
            </div>
          </div>
            ...
            <div class="picBox" v-for="(item, index) in pointList" :key="index">
              <div class="boxPhoto">
                <img :src="item.imagePath" alt="" @click="selected(index, item)"
                  :class="{ 'borderWhite': index === active }" :style="{ 'cursor': 'pointer' }">
                ...
                <i class="el-icon-close err" :style="{ 'display': index === active ? 'block' : 'none' }"
                  @click="removeDialog(item)"></i>
              </div>
          ...
          <!-- 图片预览弹框 -->
          <el-dialog class="picProview" :title="namePicDif === '1' ? '图片预览' : '全景图预览'" :visible="proviewPic"
            @close="proviewPic = false" :append-to-body="true" width="70%">
            <img :src="proLookPic" alt="" :style="{ 'width': '100%', 'height': '100%' }">
          </el-dialog>
          <!-- 视图导出弹窗 -->
          <viewDialog :item="dialogFlag" @closeDia="closeDia2" @noBorder="noBorder" :delItem="delFlag"
            :delInfo="delInfo" @pointListSon="pointListSon" :taskid="taskId" @runListPoint="runListPoint">
          </viewDialog>
        </div>
        ...
        <div class="view_photo view_animation" v-if="viewPic === '2'">
          ...
          <div class="search">
            <el-input class="searchInput" type="text" v-model="inputTwo" placeholder="请输入视点动画名称"
              @keyup.enter.native="searchAnim">
              <div slot="prefix"><i class="el-icon-search" @click="searchAnim"></i></div>
            </el-input>
            <div class="threeLogo">
              <img :src="require('@/assets/images/view/plus.png')" @click="plusProEdit" alt=""
          ...
          <div class="videos">
            <div class="videosList" v-for="(item, index) in viewPointLists" :key="index">
              <div class="frontCover">
                <img v-if="item.imagePath" @click="picAnimation(item, index)"
                  :class="{ 'animationBorder': activeAnimation === index }" :src="item.imagePath" alt="">
                <img v-else @click="picAnimation(item, index)" :class="{ 'animationBorder': activeAnimation === index }"
                  :src="require('@/assets/images/view/picFirst.png')" alt="" :style="{ 'border': '1px #fff solid' }">
              </div>
              <div class="videoDes">
                <div class="upWordes">
                  <span>时&nbsp;&nbsp;&nbsp;长:&nbsp;&nbsp;</span>
              ...
              <div class="videosEdit">
                <i v-if="item.playFlags === '1' && item.imagePath" class="el-icon-video-play"
                  @click="startPlay(item, index)"></i>
                <i v-if="item.playFlags === '2' && item.imagePath" class="el-icon-video-pause"
                  @click="logoClick('pause', item, index)"></i>
                <i v-if="item.playFlags === '3' && item.imagePath" class="el-icon-remove-outline"
                  @click="logoClick('replay', item, index)"></i>
        ...
        <div class="proEdit" v-if="activeAnimation !== -1 && viewPic === '2'"
          :style="{ right: isShow ? 'calc(-100vw + 600px)' : 'calc(-100vw + 300px)' }">
          <div class="proEditMain">
            <div class="proEditTop" onselectstart="return false;">
              ...
              <div class="play">
                <div class="leftPlay" @mousedown="speedUp(true)" @mouseup="speedStop()"
                  :class="animaViewPointer === undefined || animaViewPointer.length < 2 ? 'noAllowed' : ''">
                  <i class="el-icon-caret-left" :style="{ 'font-size': '16px' }"></i>
                  <i class="el-icon-caret-left"></i>
                </div>
                <i v-if="playFlags === '1'" class="el-icon-video-play" @click="startPlay"
                  :class="animaViewPointer === undefined || animaViewPointer.length < 2 ? 'noAllowed' : ''"></i>
                ...
                <div class="rightPlay" @mousedown="speedUp()" @mouseup="speedStop()"
                  :class="animaViewPointer === undefined || animaViewPointer.length < 2 ? 'noAllowed' : ''">
              ...
              <!-- 播放条 -->
              <div class="startPost" v-if="animaViewPointer.length >= 2" :class="playFlags === '2' ? 'noAllowed' : ''"
                style="left: 6px;">
                <div class="outCircle">
                </div>
            ...
            <div class="proEditDown">
              <div class="allWidth" :style="{ 'display': 'flex' }" onselectstart="return false;">
                <draggable v-model="animaViewPointer" handle=".dragImg"
                  :options="{ scroll: true, animation: 800, forceFallback: true, fallbackOnBody: true, dragClass: 'dragClass' }"
                  @start="onStart" @end="onEnd">
                  <transition-group :style="{ 'display': 'flex' }">
                    <div class="viewMorePic" v-for="(item, index) in animaViewPointer" :key="item.tid">
                      <img class="dragImg" :src="item.imagePath" alt="" @click="selectPoints(item, index)"
                        :class="{ 'pointBor': activePoints === index }"
                        :style="{ 'width': '100%', 'height': '100%', 'cursor': 'move' }">
                      <i class="el-icon-close pointsClose" @click="delPoints(index, item)"
                        v-if="activePoints === index"></i>
                      <div class="videosPlus">
                        <img :src="require('@/assets/images/view/jiahao.png')" @click.stop="addView(index)"
                          :style="{ 'width': '100%', 'height': '100%' }" alt="">
                      </div>
                      <div class="videoWords" v-if="index <= animaViewPointer.length - 2 && animaViewPointer.length > 1"
                        @click="changePointTime(item)">{{ item.time | timeChanger }}</div>
                      <div class="firstAdd" v-if="index === 0">
                        <img :src="require('@/assets/images/view/jiahao.png')" @click.stop="addView('one')"
                          :style="{ 'width': '100%', 'height': '100%' }" alt="">
                      </div>
                    </div>
                  </transition-group>
